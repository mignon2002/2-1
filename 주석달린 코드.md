...
for raw in g :
    number.append(raw[0]) #number 리스트에 첫번째 열을 모두 더해준다 (문항 번호 list 생성)
    answer.append(raw[1]) #answer 리스트에 두번째 열을 모두 더해준다 (답안 키워드 list 생성)
    score.append(raw[2]) #score 리스트에 세번째 열을 모두 더해준다 (배점 list 생성)
